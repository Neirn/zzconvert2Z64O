...
import { ZZManifestHelper } from './ZZManifestHelper';
  ...
  private OOT() {
    ...
    if (zz.OOT.adult_model.length > 0) {
      for (let i = 0; i < zz.OOT.adult_model.length; i++) {
        ...
        let helper = new ZZManifestHelper(path.resolve(__dirname, zz.OOT.adult_manifest), fse.readFileSync(path.resolve(__dirname, zz.OOT.adult_model[i].file)),  zz.OOT.adult_model[i].file, this.ModLoader);
        // fse.writeFileSync(path.resolve('./adult_test.zobj'), helper.getZobj());
        // helper.printDictionary();
        let evt = new Z64Online_ModelAllocation(helper.getZobj(), Age.ADULT);
    ...
    if (zz.OOT.child_model.length > 0) {
      for (let i = 0; i < zz.OOT.child_model.length; i++) {
        ...
        let helper = new ZZManifestHelper(path.resolve(__dirname, zz.OOT.child_manifest), fse.readFileSync(path.resolve(__dirname, zz.OOT.child_model[i].file)), zz.OOT.child_model[i].file, this.ModLoader);
        // fse.writeFileSync(path.resolve('./child_test.zobj'), helper.getZobj());
        // helper.printDictionary();
        let evt = new Z64Online_ModelAllocation(helper.getZobj(), Age.CHILD);
